...
public class Registeration {
	...
	public static void passValueToField(FIELDS field) {
		...
		try {
			...
			if(field.getType() == FieldsExcelConstants.FIELD_TYPE_DROPDOWN) {
				Select dropdown = new Select(driver.findElement(By.id(field.getId())));
				List<WebElement> options = dropdown.getOptions();
				String expectedOption = field.getValue().toLowerCase();
				for (WebElement option : options) {
					String currentOption = option.getText().toLowerCase();
				    if (currentOption.contains(expectedOption)) {
				        option.click();
				        isOptionSelected = true;
				        System.out.print("Selected value = " + option.getText() + " , Expected = " + field.getValue());
				        break;
				    }
				}
			...
			if(isOptionSelected) {
				System.out.print("Warning -- Option not found: " + field.getId());
			} else {
	...
	private static void fillAllFields() {
		try {
			if (ReadCustomerExcel.getDataFromExcel()) {
				...
				String farmerName = FieldsExcelConstants.FIELDS_MAP.get("Farmer Name").getValue();
				...
				System.out.println("********************* Started form filling for " + farmerName + " *********************");
